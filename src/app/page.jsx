...
const toggleFavorite = async (carId) => {
  ...
  if (favorites.includes(carId)) {
    ...
    try {
      ...
      if (!favItem) {
        console.warn("Не найден объект избранного для удаления");
        return;
      }
            ...
            <div key={car._id} className="border border-gray-500 bg-[#545454] p-1 rounded-lg shadow-md hover:shadow-xl transition cursor-pointer">
              ...
              <div className="flex gap-2 mt-2">
                ...
                <div
                  ...
                  className={`rounded-lg w-[30%] flex items-center justify-center ${favorites.includes(car._id) ? '' : 'text-white'}`}>
                  {favorites.includes(car._id) 
                    ? <IoIosHeart color='red' size={25}/> 
                    : <IoIosHeartEmpty color='black' size={25}/>}
                </div>
              </div>
            </div>
          );
